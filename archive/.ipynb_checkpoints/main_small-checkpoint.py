...
if __name__ == "__main__":
    ...
    p.add_argument('--data_type', type=str, required=True,
                   help='Type of the data, original or same lifetime')
    ...
    p.add_argument('--T_threshold_low', type=int, default=43200, required=False,
                   help='All eventfiles are turncated to the same length')
    ...
    starting_times = pd.read_csv(f'{root_dir}/Chandra_data/starting_time.csv')
    ...
    elif opt.data_type == 'same':
        data_lst = []
        T_threshold_low = opt.T_threshold_low
        num_flares = 436
        i_flare = 0
        for key in list(d.keys()):
            event_list = d[key]
    ...
    data = RealEventsDataset(data_lst,t_scale=opt.t_scale)
